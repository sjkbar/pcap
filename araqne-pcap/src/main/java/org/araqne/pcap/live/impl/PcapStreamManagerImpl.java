 ...
 */
package org.araqne.pcap.live.impl;
...
import org.araqne.pcap.live.PcapStreamEventListener;
...
public class PcapStreamManagerImpl implements PcapStreamManager {
	...
	public void start(String key, String deviceName, int milliseconds, Promiscuous promisc, String filter)
		...
		logger.info("araqne-pcap: starting live runner [{}]", key);
		...
		for (PcapStreamEventListener callback : callbacks) {
			try {
				...
				logger.warn("araqne pcap: callback should not throw any exception", e);
			}
		}
	}
	...
	public void stop(String alias) {
		logger.info("araqne-pcap: stopping live runner [{}]", alias);
