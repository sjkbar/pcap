 ...
 */
package org.araqne.pcap.http.impl;
...
import org.araqne.pcap.http.HttpHeaders;
import org.araqne.pcap.http.HttpResponse;
import org.araqne.pcap.http.HttpVersion;
				...
				try {
					if (decompressedGzip == null) {
						...
						if (logger.isDebugEnabled())
							logger.debug("araqne http decoder: gzip decoding failed");
						textContent = null;
					} else {
						if (charset != null)
					...
					if (logger.isDebugEnabled())
						logger.debug("araqne http decoder: unsupported encoding", e);
				}
